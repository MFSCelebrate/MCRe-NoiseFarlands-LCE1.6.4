...
void CPlatformNetworkManagerStub::DoWork()
{
		...
		while (WinsockNetLayer::PopDisconnectedSmallId(&disconnectedSmallId))
		{
			...
			if (qnetPlayer != nullptr && qnetPlayer->m_smallId == disconnectedSmallId)
			{
				...
				while (IQNet::s_playerCount > 1 &&
					IQNet::m_player[IQNet::s_playerCount - 1].m_smallId == 0 &&
					WinsockNetLayer::GetSocketForSmallId(IQNet::s_playerCount - 1) == INVALID_SOCKET)
					IQNet::s_playerCount--;
			}
	...
	if (m_bJoinPending)
	{
		...
		if (state == WinsockNetLayer::eJoinState_Success)
		{
			...
			IQNet::m_player[localSmallId].m_resolvedXuid = Win64Xuid::ResolvePersistentXuidFromName(IQNet::m_player[localSmallId].m_gamertag);
